...
def get_registration_ext_model():
    ...
    if hasattr(settings, "REGISTRATION_EXTENSION_FORM"):
        try:
            ...
            module_path = ".".join(form_class_path.split(".")[:-1])
            module = importlib.import_module(module_path)
            form_class = getattr(module, form_class_path.split(".")[-1])
...
class Command(BaseCommand):
    ...
    help = "Exports users email opt-in preferences to an s3 bucket"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "when enabled the script skips the row when the value of"
                " one of the fields is null (default=True)"
            )
        )
        parser.add_argument(
            "-ns",
        ...
        parser.add_argument(
            "--aws-profile", type=str, help="specify which aws profile to use"
        )
        parser.add_argument(
            "--fields",
            type=str,
        ...
        parser.add_argument(
            "--usage", action="store_true", help="print the module docstring header"
        )
        parser.add_argument(
            "--s3-bucket-name",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        query = construct_query(options["fields"])
        ...
        try:
            s3_client.Bucket(  # pylint: disable=no-member
                options["s3_bucket_name"]
            ).put_object(Key=options["s3_object_name"], Body=binary_fd)
